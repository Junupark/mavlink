...
#define MAVLINK_MSG_ID_ASCL_COMMAND_ACK_CRC 218
#define MAVLINK_MSG_ID_53250_CRC 218
...
#define MAVLINK_MESSAGE_INFO_ASCL_COMMAND_ACK { \
    ...
    "ASCL_COMMAND_ACK", \
    2, \
    {  { "command", NULL, MAVLINK_TYPE_UINT16_T, 0, 0, offsetof(mavlink_ascl_command_ack_t, command) }, \
...
{
    return mavlink_msg_ascl_command_ack_pack(system_id, component_id, msg, ascl_command_ack->command, ascl_command_ack->result);
}
...
{
    return mavlink_msg_ascl_command_ack_pack_chan(system_id, component_id, chan, msg, ascl_command_ack->command, ascl_command_ack->result);
}
...
static inline void mavlink_msg_ascl_command_ack_send(mavlink_channel_t chan, uint16_t command, uint8_t result)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_ASCL_COMMAND_ACK_LEN];
    _mav_put_uint16_t(buf, 0, command);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_ascl_command_ack_send(chan, ascl_command_ack->command, ascl_command_ack->result);
#else
 ...
 */
static inline void mavlink_msg_ascl_command_ack_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint16_t command, uint8_t result)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field command from ascl_command_ack message
 *
 * @return  Command ID
 */
static inline uint16_t mavlink_msg_ascl_command_ack_get_command(const mavlink_message_t* msg)
{
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    ascl_command_ack->command = mavlink_msg_ascl_command_ack_get_command(msg);
